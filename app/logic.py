...
def process_login(code):
    ...
    that_inst_profile = db.InstProfile.session.query.filter_by(id_profile=instagram_user['id']).first()
    ...
    that_user = db.User.session.query.filter_by(id_user=that_inst_profile).first()
    if (that_user == None):
...
def get_inst_profile(user_id):
    ...
    user = db.User.session.query.filter_by(id_user=inst_profile).first()
