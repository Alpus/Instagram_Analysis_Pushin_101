...
def process_login(code):
    ...
    that_inst_profile = models.InstProfile.session.query.filter_by(id_profile=instagram_user['id']).first()
    ...
    that_user = models.User.session.query.filter_by(id_user=that_inst_profile).first()
    if (that_user == None):
...
def get_inst_profile(user_id):
    inst_profile = models.InstProfile,session.query.filter_by(id_profile=user_id).first()
    user = models.User.session.query.filter_by(id_user=inst_profile).first()
