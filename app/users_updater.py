...
def update_users_media():
    while True:
        users = db.session.query(models.User).filter(models.User.access_token != None,
                                                     models.User.is_media_on_update == False)\
                                                     .order_by(models.User.last_check).all()
        for user in users:
            ...
            if datetime.datetime.now() - user.last_check > datetime.timedelta(hours=24):
                ...
                db.session.commit()
                requests.update_user_media.delay(user.inst_id_user)
