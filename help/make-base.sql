...
CREATE TABLE IF NOT EXISTS `InstagamBase`.`Users` (
  ...
  PRIMARY KEY (`id_user`)
  FOREIGN KEY (`id_user`) REFERENCES `InstagamBase`.`InstProfiles` (`id_user`),
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `InstagamBase`.`Follows` (
  ...
  FOREIGN KEY (`id_follower`) REFERENCES `InstagamBase`.`InstProfiles` (`id_user`),
  FOREIGN KEY (`id_following`) REFERENCES `InstagamBase`.`InstProfiles` (`id_user`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `InstagamBase`.`ProfileGeos` (
  `id_usergeo` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `id_user` INT UNSIGNED NOT NULL,
  `id_geo` INT UNSIGNED NOT NULL,
  `using_count` INT UNSIGNED NULL,
  `get_like` INT UNSIGNED NULL,
  `get_comment` INT UNSIGNED NULL,
  PRIMARY KEY (`id_usergeo`),
  UNIQUE KEY ('id_user', 'id_geo'),
  FOREIGN KEY (`id_user`) REFERENCES `InstagamBase`.`InstProfiles` (`id_user`),
...
CREATE TABLE IF NOT EXISTS `InstagamBase`.`ProfileWords` (
  `id_userword` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `id_user` INT UNSIGNED NOT NULL,
  `id_word` INT UNSIGNED NOT NULL,
  `using_count` INT UNSIGNED NULL,
  `is_tag` TINYINT(1) NOT NULL,
  PRIMARY KEY (`id_userword`),
  UNIQUE KEY (`id_user`, `id_geo`, `is_tag`),
  FOREIGN KEY (`id_user`) REFERENCES `InstagamBase`.`InstProfiles` (`id_user`),
...
CREATE TABLE IF NOT EXISTS `InstagamBase`.`Posts` (
  ...
  `last_check` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id_post`),
  FOREIGN KEY (`id_user`) REFERENCES `InstagramBase`.`InstProfiles` (`id_user`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `InstagamBase`.`ProfileFilters` (
  `id_userfilter` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `id_user` INT UNSIGNED NOT NULL, 
  `id_filter` INT UNSIGNED NOT NULL,
  `using_count` INT UNSIGNED NULL,
  `get_like` INT UNSIGNED NULL,
  `get_comments` INT UNSIGNED NULL,
  PRIMARY KEY (`id_userfilter`),
  UNIQUE KEY (`id_user`, `id_filter`),
  FOREIGN KEY (`id_user`) REFERENCES `InstagamBase`.`InstProfiles` (`id_user`),
...
CREATE TABLE IF NOT EXISTS `InstagamBase`.`ProfileUsermarks` (
  ...
  UNIQUE KEY (`id_user`, `id_mark`),
  FOREIGN KEY (`id_user`) REFERENCES `InstagamBase`.`InstProfiles` (`id_user`),
  FOREIGN KEY (`id_mark`) REFERENCES `InstagamBase`.`InstProfiles` (`id_user`)))
ENGINE = InnoDB;